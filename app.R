  ...
  sidebarLayout(
    sidebarPanel(width = 4,
                 ...
                 conditionalPanel("input.metodo == 'umbrales'",
                                  helpText("Ajuste los umbrales: t1 (BAJO→MEDIO) y t2 (MEDIO→ALTO) con las regletas."),
                                  
                                  sliderInput("sl_bio", "Biomasa — umbrales (t1, t2)",
                                              min = 0, max = 1, value = c(0.33, 0.66), step = 0.01, width = "100%"),
                                  sliderInput("sl_clo", "Clorofila — umbrales (t1, t2)",
                                              min = 0, max = 1, value = c(0.33, 0.66), step = 0.01, width = "100%"),
                                  sliderInput("sl_est", "Estrés — umbrales (t1, t2)",
                                              min = 0, max = 1, value = c(0.33, 0.66), step = 0.01, width = "100%"),
                                  sliderInput("sl_nit", "Nitrógeno — umbrales (t1, t2)",
                                              min = 0, max = 1, value = c(0.33, 0.66), step = 0.01, width = "100%")
                 ),
...
server <- function(input, output, session) {
  ...
  # Configura límites/step y valores sugeridos de un slider a partir de los datos
  slider_config <- function(v, param) {
    v <- suppressWarnings(as.numeric(v))
    v <- v[is.finite(v)]
    # Defaults por si no hay datos suficientes:
    if (length(v) < 3) {
      return(list(min = 0, max = 1, step = 0.01, value = c(0.33, 0.66)))
    }
    minv <- min(v); maxv <- max(v)
    if (!is.finite(minv) || !is.finite(maxv) || minv == maxv) {
      minv <- minv - 0.5; maxv <- maxv + 0.5
    }
    rng  <- maxv - minv
    step <- signif(rng/100, 2); if (!is.finite(step) || step <= 0) step <- 0.01
    
    # Valores sugeridos en base a tu criterio (0–1) y transformados a la escala real
    cut01 <- suggested_cutpoints_01(param)
    sugg  <- backtransform_cuts(v, cut01[1], cut01[2])
    t1 <- max(minv, min(maxv, sugg[1]))
    t2 <- max(minv, min(maxv, sugg[2]))
    
    list(min = minv, max = maxv, step = step, value = c(t1, t2))
  }
  
  # Inicializa sliders de umbrales manuales según los datos presentes (una sola vez)
  observeEvent(TRUE, {
    # BIOMASA
    cfg <- slider_config(plantas_pts$biomasa, "biomasa")
    updateSliderInput(session, "sl_bio", min = cfg$min, max = cfg$max,
                      value = cfg$value, step = cfg$step)
    
    # CLOROFILA
    cfg <- slider_config(plantas_pts$clorofila, "clorofila")
    updateSliderInput(session, "sl_clo", min = cfg$min, max = cfg$max,
                      value = cfg$value, step = cfg$step)
    
    # ESTRÉS
    cfg <- slider_config(plantas_pts$estres, "estres")
    updateSliderInput(session, "sl_est", min = cfg$min, max = cfg$max,
                      value = cfg$value, step = cfg$step)
    
    # NITRÓGENO
    cfg <- slider_config(plantas_pts$nitrogeno, "nitrogeno")
    updateSliderInput(session, "sl_nit", min = cfg$min, max = cfg$max,
                      value = cfg$value, step = cfg$step)
  }, once = TRUE)   # <- observeEvent sí acepta 'once'
  ...
  r_classified <- reactive({
    ...
    clasif <- function(v, p) {
      ...
      if (input$metodo == "cuantiles") classify_bma(v, "cuantiles")
        ...
        # Lee (ordenados) los valores de sliders por parámetro
        rng <- switch(p,
                      biomasa   = sort(input$sl_bio  %||% c(NA, NA)),
                      clorofila = sort(input$sl_clo  %||% c(NA, NA)),
                      estres    = sort(input$sl_est  %||% c(NA, NA)),
                      nitrogeno = sort(input$sl_nit  %||% c(NA, NA))
        )
        t1 <- rng[1]; t2 <- rng[2]
  ...
  output$hist_param <- renderPlot({
    ...
    if (sel == "Índice compuesto") {
      ...
      t1t2 <- switch(sel,
                     Biomasa    = sort(input$sl_bio  %||% c(NA, NA)),
                     Clorofila  = sort(input$sl_clo  %||% c(NA, NA)),
                     `Estrés`   = sort(input$sl_est  %||% c(NA, NA)),
                     `Nitrógeno`= sort(input$sl_nit  %||% c(NA, NA))
      )
      t1 <- t1t2[1]; t2 <- t1t2[2]
      if (!is.finite(t1) || !is.finite(t2)) {
        cuts <- as.numeric(quantile(v_all, probs = c(1/3, 2/3), na.rm = TRUE))
